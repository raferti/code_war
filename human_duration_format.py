...
by a space. The unit of time is used in plural if the integer
is greater than 1.

The components are separated by a comma and a space (", "). Except the last
component, which is separated by " and ", just like
it would be written in English.
...
the duration specified by of a component must not be greater than any
...
def format_duration(seconds):
    ...
    hh, mm = divmod(mm, 60)
    dd, hh = divmod(hh, 24)
    year, dd = divmod(dd, 365)

    list_format_date = []
    name_time = ['year', 'day', 'hour', 'minute', 'second']
    time = [year, dd, hh, mm, ss]

    for num, value in enumerate(name_time):
        ending = ''
        if time[num] == 0:
            continue
        elif time[num] > 1:
            ending = 's'
        list_format_date.append('{} {}{}'.format(time[num], value, ending))

    return ' and '.join(list_format_date).replace(' and ', ', ',
                                                  len(list_format_date)-2)
